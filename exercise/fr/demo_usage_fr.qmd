...
L'objectif de cet exercice est d'explorer les fonctionalités et l'utilisation d'une linelist rougeole
...
#### 1- Importer des données, découvrir la feuille de saisie de données. 
...
Ouvrir la feuille `linelist_measles_usage_fr.xlsb`. 

Dans l'onglet `OutbreakTools`, dans la section `Imports`, cliquez
sur le bouton `Importer des données`. Naviguez et sélectionnez le fichier `import_linelist_fr.xlsb` puis validez.

A la fin de l'import, on vous demandera si vous voulez voir un rapport. Sélectionnez "Oui".
...
Grâce au bouton `Aller à la section` à l'onglet `$C$2`, sélectionnez
la section `Caractéristiques de l'âge des patients`.

Parmi les patients non vaccinés, quel est le groupe d'âge le plus
représenté?

Y a t-il des patients adultes (de plus de 15 ans) décédés?

Quand ces patients sont-ils décédés?
...
*Vous pourrez utiliser les filtres dans la feuille `Linelist patients` pour répondre à ces question*

Quelle est la létalité des moins de 5 ans au cours de la semaine épidémiologique où l'on a eu le plus de cas? 

Dans quelle semaine épidémiologique as-ton enregistré le plus de décès?
...
D'où vient la plupart des patients qui décèdent?
...
Faites de mêmes et faites une `Exportation anonyme pour MSF`. C'est 
cette exportation qui est envoyée au dashboard. Il ne faut pas la renommer. Vous pouvez l'ouvrir pour explorer les données et regarder
les différences entre les deux exports. Le mot de passe est 1234.
