...
The configuration of the file is inspired from Kobo's XLS forms. No in-depth knowledge of Excel is required. A setup file is defined by disease, and can create linelists in several languages.
...
- Open the setup file `setup_measles_base_ex2.xlsb`. We want to add the following variables:
  ...
  - In the _Vaccination_ section, just after _Vaccination controls measles_:
...
- In the analyses sheet, add a temporal table showing the evolution of discharge types by notification date. Add a percentage option in rows, and do not add a total.
