...
The goal of this exercise is to explore the functionality and use of a measles linelist
...
You can 
to get an idea of all the features available by reading the user guide. 
We'll just go through a few of them in the linelist. The main ones are presented in the following two diagrams:
...
You have a linelist named `linelist_measles_usage_en.xlsb`, and an excel s
sheet containing data called `import_linelist_en.xlsb`.
We're going to explore the linelist after importing the data from the sheet, and answer some questions.
...
On the `OutbreakTools` tab, click on `Show/Hide Variables` button.
...
::: {.callout-note title="Filling the Linelist"}
  ...
  *The GeoHelper*
  ...
  The GeoHelper can help in entering geographical informations in a cascade way.



  *Custom dropdowns*

  You can use custom dropdowns prefill some dropdowns on your own, base on the
...
Enter this data in the linelist.
...
the section `Patient Age Characteristics`.
